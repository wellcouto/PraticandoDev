        ...
        <header>
            ...
            <nav>
                <div class='desktopMenu'>
                    <a href="../home/home_admin.php">Home</a>
                </div>
            </nav>
        </header>
        <main>
            ...
            <div class='itens'>
                ...
                $sql = $pdo->prepare("select * from tbl_compra");
                $result = $sql->execute();
                $rowtabelaPedido = $sql->fetchAll();
                ...
                if ($sql->rowCount() > 0) {
                    foreach ($rowtabelaPedido as $rowPedido) {
                        ...
                        <div class='text'>
                            ...
                            <h3>" . $rowPedido['email'] . "</h3>
                            <h4> R$" . $rowPedido['total'] . "</h4>
                        </div> 
                        <div class='buttons'>
                            <a href='editar_pedido.php?id=".$rowPedido["id"]."' >Editar</a>
                            <a class='delete' href='deletar_produto.php?id=" . $rowPedido["id"] . "' >Excluir</a>
                        </div>
                    </div>";
                    }
                } 
    ...
    $(document).ready(function () {
        $('a.delete').click(function (e) {
            ...
            $('#confirm-modal').modal('show').one('click', '#confirm-delete', function () {
                $.ajax({
                    url: '../../controller/deletar/deletar_pedido.php',
                    type: 'GET',
                    data: { id: id },
                    success: function () {
                        location.reload();
